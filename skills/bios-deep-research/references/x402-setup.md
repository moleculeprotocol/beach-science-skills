...
Any wallet that can produce EIP-712 signatures works:

| Method | How |
|--------|-----|
| **Private key** | Sign locally with viem, ethers.js, web3.py, or eth_account |
| **Coinbase CDP** | Use `cdp-sdk` with `sign_typed_data()` |
| **Browser wallet** | MetaMask, Rabby, etc. via `eth_signTypedData_v4` |
| **MPC / multisig** | Any signer that produces a valid EIP-712 signature |
...
# For Coinbase CDP wallet instead:
...
For automation, configure your signer via environment variables:
